...
*/
	memset(dest_mem_map, 0, BUFFER_BLOCK_WIDTH * num_descr);
	/*
...
*/
	...
	munmap(dest_mem_map, BUFFER_BLOCK_WIDTH * num_descr);
	close(socket_desc);
	close(fd);
...
int  dma_alloc_mem(int fd, unsigned int *axi_dma, unsigned int *s2mm_descriptor_register_mmap, unsigned int *dest_mem){
	axi_dma = mmap(NULL, DESCRIPTOR_REGISTERS_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, AXI_DMA_BASEADDR);
	...
	s2mm_descriptor_register_mmap = mmap(NULL, DESCRIPTOR_REGISTERS_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, HP0_S2MM_DMA_DESCRIPTORS_ADDRESS);
	...
	dest_mem = mmap(NULL, DEST_MEM_BLOCK, PROT_READ | PROT_WRITE, MAP_SHARED, fd, (off_t) (HP0_S2MM_TARGET_MEM_ADDRESS));
...
int udp_send(int sock, const void *buffer, const size_t buflen){
	...
	size_t sendlen = MIN(buflen, 8000);
...
void dma_descr_set(unsigned int *s2mm_descr_vrt_address, int num_descr){
	...
	for( i = 0; i < num_descr; i++){
		...
		if( i == 0 ){
			dma_set(s2mm_descr_vrt_address, CONTROL + offset_nxtdesc, 0x8000000 | BUFFER_BLOCK_WIDTH);
		}
		else if ( i == ( num_descr - 1 )){
			dma_set(s2mm_descr_vrt_address, NXTDESC + offset_nxtdesc, HP0_S2MM_DMA_DESCRIPTORS_ADDRESS);
			dma_set(s2mm_descr_vrt_address, CONTROL + offset_nxtdesc, 0x4000000 | BUFFER_BLOCK_WIDTH);
		}
		else{
	...
	for( i = 0; i < num_descr; i++){
		...
		s2mm_descr_st = dma_get(s2mm_descr_vrt_address, STATUS + offset_status);
			printf("%d) Descriptor Status: 0x%08x \r\n"
					"\tCmplt: %x;\r\n"
					"\tRXEOF: %x;\r\n"
					"\tRXSOF: %x;\r\n", i, s2mm_descr_st, s2mm_descr_st & 0x80000000, s2mm_descr_st & 0x4000000, s2mm_descr_st & 0x8000000 );
	}
}
