...
int main(void) {
	...
	unsigned int *axi_dma_vrt = mmap(NULL, DESCRIPTOR_REGISTERS_SIZE, PROT_READ
			               | PROT_WRITE, MAP_SHARED, fd, AXI_DMA_BASEADDR);
	...
	uint32_t s2mm_tail_descr_addr = HP0_S2MM_DMA_DESCRIPTORS_ADDRESS +
			                (num_descr * 0x40 - 0x40);
...
int udp_send(int sock, const void *buffer, size_t buflen) {
	...
	// We check our size of buffer of spidev, and send by packets of determined length
	...
	ssize_t len;
	for (i = 0; i < count; i++) {
		if (lost && ((count - i) == 1)) { // last packet with remainder
			len = sendto(sock, buffer + i * UDP_BUFSIZE, buflen, 0,
			            (struct sockaddr *) &forward_ip, peer_addr_len);
			if (len == -1) {
				result_lenght = -1;
			...
			len = sendto(sock, buffer + i * UDP_BUFSIZE, UDP_BUFSIZE + 8, 0,
			            (struct sockaddr *) &forward_ip, peer_addr_len);
			if (len == -1) {
				result_lenght = -1;
				return result_lenght;
